...
#Conv1D first, then concrete dropout
...
#dense goes in first then concrete dropout
...
def graph_attn_gate(A, X, attn, out_dim, label, length, num_train):
    ...
    for i in range(len(attn)):
        ...
        print(_A.get_shape())
        print(_h.get_shape())
        print(tf.matmul(_A, _h).get_shape())
        D=tf.reduce_sum(A,axis=0)[0]
        D=(tf.diag(D))
        print(D.get_shape())
        D_inv=tf.reshape(D**-1,[-1,503,503])
        print(D_inv.get_shape())
        print((tf.matmul(D_inv,tf.matmul(_A, _h,))).get_shape())
        print(tf.matmul(_A, _h,).get_shape())
    ...
    output = tf.layers.conv1d(_X,
                          filters=out_dim,
                          kernel_size=1,
                          use_bias=False,
                          activation=None,
                          kernel_initializer=tf.contrib.layers.xavier_initializer(),
                          bias_initializer=tf.contrib.layers.xavier_initializer()) + tf.layers.conv1d(X,
                          filters=out_dim,
                          kernel_size=1,
                          use_bias=False,
                          activation=None,
